     ...
     .navbar {
        ...
        <div class="navbar-header">
           <h3>Admin Dashboard</h3>
                   ...
                   // $cdate=date("Y-m-d");
                   $name2= "SELECT sum(total_amount) FROM bill";
                  ...
                  date_default_timezone_set("Asia/Calcutta");
                  $month=date('m');
                  $name2= "SELECT sum(total_amount) FROM bill where month(bill_date)='$month' ORDER BY bill_number DESC";
                  $query=mysqli_query($con,$name2);
...
$query = "SELECT SUM(total_amount) AS total_sales FROM bill WHERE DATE(bill_date) = '$todayDate'";
...
// Query to count the total number of products
$query = "SELECT COUNT(*) AS total_products FROM bill";
$result = mysqli_query($con, $query);
if ($result) {
    $row = mysqli_fetch_assoc($result);
    $totalProducts = $row['total_products'];
